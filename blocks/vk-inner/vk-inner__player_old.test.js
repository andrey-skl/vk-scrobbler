describe('OLD Vk-inner player', function () {
  'use strict';
  var ARTIST_NUM = 5;
  var TOTAL_NUM = 3;
  var TITLE_NUM = 6;
  var patcher;
  var PlayerPatcherOld = window.vkScrobbler.PlayerPatcherOld;

  beforeEach(function () {
    this.sinon = sinon.sandbox.create();
    this.sinon.stub(PlayerPatcherOld.prototype, 'waitForPlayerAndPatch');
    patcher = new PlayerPatcherOld();
  ...
  describe('callListener', function () {
    ...
    beforeEach(function () {
      ...
      PlayerPatcherOld.addCallListener(fakeObj, 'method', handler);
    });
    ...
    it('Should support function passing and call it before', function () {
      ...
      PlayerPatcherOld.addCallListener(fakeObj, 'method', before);
      fakeObj.method('foo', {bar: 'test'});
  ...
  describe('player patching', function () {
    ...
    describe('waiting and patching window.audioPlayer', function () {
      beforeEach(function () {
        PlayerPatcherOld.prototype.waitForPlayerAndPatch.restore();
        this.clock = this.sinon.useFakeTimers();
      });

      it('Should wait for audioPLayer in window and patch it', function () {
        var p = new PlayerPatcherOld();
